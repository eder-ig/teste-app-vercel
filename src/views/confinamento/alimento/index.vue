<template>
  <pagina
    ...
    titulo-toolbar="Informações do Alimento"
    titulo="Alimentos"
    fechar
          ...
          <v-container fluid grid-list-xs>
            <v-row dense>
              ...
              <v-col cols="12">
                <validation-provider
                  v-slot="{ errors }"
                  name="Descrição Produto IGFAZ"
                  rules="required"
                  vid="descricaoSubgrupoIgfaz"
                >
                  <v-text-field
                    v-model="formulario.descricaoSubgrupoIgfaz"
                    v-mask="mascara('T', 250)"
                    :disabled="controle.exibir"
                    :error-messages="errors"
                    :hide-details="!errors.length"
              ...
              <v-col cols="12">
                <validation-provider
                  v-slot="{ errors }"
                  name="Código Produto IGFAZ"
                  vid="produtoIgfazId"
                >
                  <v-text-field
                    v-model="formulario.produtoIgfazId"
                    v-mask="mascara('T', 250)"
                    :disabled="controle.exibir"
                    :error-messages="errors"
                    :hide-details="!errors.length"
                    dense
                    label="Código Produto IGFAZ"
                    outlined
                  />
                </validation-provider>
              </v-col>
...
export default {
  name: "PaginaConfinamentoAlimento",
  ...
  data: () => ({
    ...
    colunas: [
      ...
      {
        text: "Descrição Produto IGFAZ",
        align: "start",
        sortable: true,
        value: "descricaoSubgrupoIgfaz",
        width: 50,
      },
      {
        text: "Unidade",
      ...
      {
        text: "Código Produto IGFAZ",
        align: "center",
        sortable: true,
        value: "produtoIgfazId",
      },
    ],
    modal: false,
    controle: {
  ...
  computed: {
    ...mapState("paginaConfinamentoAlimento", ["registros", "ruas"]),
    optionsFilter() {
      return {
        adicionar: true,
  ...
  methods: {
    ...mapActions("paginaConfinamentoAlimento", [
      "listar",
      "editar",
      "salvar",
      "exibir",
    ...
    async salvarRegistro() {
      if (await this.$refs.observer.validate()) {
        ...
        const form = {
          ...
          produtoIgfazId: this.formulario.produtoIgfazId || undefined,
          descricao: this.formulario.descricao || undefined,
          descricaoSubgrupoIgfaz:
            this.formulario.descricaoSubgrupoIgfaz || undefined,
          unidade: this.formulario.unidade || undefined,
        };
        let res;
    ...
    async exibirRegistro(id) {
      ...
      if (res && !res.erro) {
        this.formulario = {
          ...
          produtoIgfazId: res.produtoIgfazId || null,
          descricaoSubgrupoIgfaz: res.descricaoSubgrupoIgfaz || null,
