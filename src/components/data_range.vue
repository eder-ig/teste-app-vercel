    ...
    <validation-observer ref="observer">
      ...
      >
        <template v-slot:activator="{ on }">
          ...
          >
            <v-text-field
              ...
              @change="
                dataFormatada.length <= 13 || !dataFormatada
                  ? ((date = [$day().format('YYYY-MM-DD')]), changeData())
                  : confirmarData()
              "
            />
        ...
        >
          ...
          <v-btn color="error" @click="resetData()"> Limpar </v-btn>
        </v-date-picker>
      </v-menu>
    </validation-observer>
  </div>
  ...
  watch: {
    value(valor) {
      if (valor && valor.dataInicio && !valor.dataFim) {
        this.dataFormatada = `${this.$day(
          valor.dataInicio,
          "YYYY-MM-DD"
        ).format("DD/MM/YYYY")} à ${this.$day(
          valor.dataInicio,
          "YYYY-MM-DD"
        ).format("DD/MM/YYYY")}`;
      ...
      if (v && v.length && [10, 13].includes(v.length)) {
        ...
        if (day <= 31 && month <= 12) {
          ...
          this.$refs.observer.setErrors({
            dataFormatada: ["Formato de data inválida"],
          });
        }
        ...
        if (day1 <= 31 && day2 <= 31 && month1 <= 12 && month2 <= 12) {
          ...
          if (this.$day(data1).isAfter(data2)) {
            ...
            if (
              this.value &&
              (this.value.dataInicio !== this.date[0] ||
                this.value.dataFin !== this.date[1])
            ) {
              this.$emit("input", {
                dataInicio: data2,
                dataFim: data1,
          ...
          !(day1 <= 31 && day2 <= 31 && month1 <= 12 && month2 <= 12)
        ) {
          this.$refs.observer.setErrors({
            dataFormatada: ["Formato de data inválida"],
          });
  ...
  methods: {
    confirmarData() {
      ...
      if (this.date) {
        ...
        if (this.date[0]) {
          dataInicial = this.$day(this.date[0], "YYYY-MM-DD").format(
            "DD/MM/YYYY"
          );
        } else {
    ...
    changeData() {
      if (
        !this.date &&
        this.dataFormatada &&
        this.dataFormatada.length &&
        [10, 13].includes(this.dataFormatada.length)
      ) {
        this.date[1] = this.date[0];
        ...
        this.date[1] = this.date[0];
        this.dataFormatada = `${this.$day(this.date[0]).format(
          "DD/MM/YYYY"
        )} à ${this.$day(this.date[1]).format("DD/MM/YYYY")}`;
      } else if (!this.date.length) {
        const data = this.$day().format("YYYY-MM-DD");
        this.date = [data, data];
        this.dataFormatada = `${this.$day().format(
          "DD/MM/YYYY"
        )} à ${this.$day().format("DD/MM/YYYY")}`;
      }
    },
