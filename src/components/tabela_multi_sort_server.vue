<template>
  <div v-resize="newCalculaTableHeigth">
    ...
    >
      ...
      <template v-slot:[`item.historico`]="{ value }">
        {{
          value && value.length >= historicoSliceSize
            ? `${value.substring(0, historicoSliceSize)} [...]`
            : value
        }}
      </template>
      ...
      <template v-slot:[`item.${departamentoTag}`]="{ value }">
        {{ value.text }}
      </template>
        ...
        >
          <td
            ...
            :class="
              ...
              `text-${col.total === 'Totais' ? 'start' : 'end'} py-1`)
            "
            :style="
              $vuetify.theme.dark
...
export default {
  ...
  computed: {
    desabilitarPaginaAnterior() {
      return this.paginacao.page === 1 || this.paginacao.page === undefined;
    },
    desabilitarPaginaPosterior() {
      return (
        this.paginacao.page === this.paginacao.totalPaginas ||
        this.paginacao.totalPaginas === 0
      );
    },
    itensServidor() {
      return this.paginacao.totalRegistros;
    },
    paginaAtual() {
    ...
    totalRegistrosRodape() {
      return moeda.format(this.paginacao.totalRegistros || 0, 0);
    },
    selected: {
      get() {
  ...
  watch: {
    ...
    paginacao: {
      handler(val) {
        if (val) {
          this.rowsPerPage = this.paginacao.itemsPerPage;
          this.page = val.page;
        }
      },
    },
  ...
  created() {
    this.page = this.paginacao.page;
    this.rowsPerPage = this.paginacao.itemsPerPage || 20;
  },
